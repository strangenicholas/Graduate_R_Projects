...
demog <-
  filter(demog, METAREA == 616 &
           OWNERSHP != 0) #filter to PA/NJ and remove NA ownership
...
demog <-
  ...
  mutate(
    ...
    industry = if_else(
      ...
      NA,
      if_else(
        IND1990 >= 1 & IND1990 <= 32,
        'Agriculture',
        if_else(
          IND1990 >= 40 & IND1990 <= 50,
          'Mining',
          if_else(
            IND1990 == 60,
            'Construction',
            if_else(
              IND1990 >= 100 & IND1990 <= 392,
              'Manufacturing',
              if_else(
                IND1990 >= 400 & IND1990 <= 472,
                'Transportation',
                if_else(
                  IND1990 >= 500 & IND1990 <= 571,
                  'Wholesale',
                  if_else(
                    IND1990 >= 580 & IND1990 <= 691,
                    'Retail',
                    if_else(
                      IND1990 >= 700 & IND1990 <= 712,
                      'Finance',
                      if_else(
                        IND1990 >= 721 & IND1990 <= 760,
                        'Business',
                        if_else(
                          IND1990 >= 761 & IND1990 <= 791,
                          'Personal Services',
                          if_else(
                            IND1990 >= 800 & IND1990 <= 893,
                            'Professional Services',
                            if_else(
                              IND1990 >= 900 & IND1990 <= 932,
                              'Public Admin',
                              if_else(IND1990 >= 940 &
                                        IND1990 <= 960, 'Military', 'Other')
                            )
                          )
                        )
                      )
    ...
    occupation = if_else(
      OCC1990 >= 3 & OCC1990 <= 200,
      'Managerial & Professional',
      if_else(
        OCC1990 >= 203 & OCC1990 <= 389,
        'Technical, Sales & Admin',
        if_else(
          OCC1990 >= 405 & OCC1990 <= 469,
          'Service',
          if_else(
            OCC1990 >= 473 & OCC1990 <= 498,
            'Farming, Forestry & Fishing',
            if_else(
              OCC1990 >= 503 & OCC1990 <= 699,
...
fitNLR <-
  lm(homeown ~  iscitizen + edulvl + ancestry + hhincome + married,
     data = demog)
...
fitLgR <-
  glm(
    homeown ~  iscitizen + edulvl + ancestry + employed + hhincome + totincome + industry + occupation + married + travtime + SEX,
    family = "binomial",
    data = demog
  )
summary(fitLgR)
